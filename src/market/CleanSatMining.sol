    ...
    ICleanSatMining
{
    ...
    modifier onlyWhitelistTokenWithType(address token_) {
        require(tokenTypes[token_] != TokenType.NOTWHITELISTEDTOKEN, "Token is not whitelisted");
        _;
    }
    ...
    function buyWithPermit(
        ...
        uint256 buyerTokenAmount = (price * amount) / (uint256(10) ** ERC20(offerTokens[offerId]).decimals());
        IBridgeToken(buyerTokens[offerId]).permit(msg.sender, address(this), buyerTokenAmount, deadline, v, r, s);
        _buy(offerId, price, amount);
    }
    ...
    function updateOfferWithPermit(
        ...
        IBridgeToken(offerTokens[offerId]).permit(msg.sender, address(this), newAllowance, deadline, v, r, s);
        ...
        returns (address, address, address, address, uint256, uint256)
    {
        ...
        uint256 availableBalance = ERC20(offerTokens[offerId]).balanceOf(sellers[offerId]);
        uint256 availableAllow = ERC20(offerTokens[offerId]).allowance(sellers[offerId], address(this));
     ...
     *
     */
    function _createOffer(address _offerToken, address _buyerToken, address _buyer, uint256 _price, uint256 _amount)
        private
        ...
        onlyWhitelistTokenWithType(_buyerToken)
    {
        ...
        emit OfferCreated(_offerToken, _buyerToken, msg.sender, _buyer, _offerId, _price, _amount);
    }
     ...
     *
     */
    function _updateOffer(uint256 _offerId, uint256 _price, uint256 _amount) private {
        require(sellers[_offerId] == msg.sender, "only the seller can change offer");
        emit OfferUpdated(_offerId, prices[_offerId], _price, amounts[_offerId], _amount);
        prices[_offerId] = _price;
        amounts[_offerId] = _amount;
    }
    ...
    function _buy(uint256 _offerId, uint256 _price, uint256 _amount) private {
        ...
        require(block.number > offerBlockNumbers[_offerId], "can not buy in the same block as offer creation");
        ...
        require(_amount * _price > (uint256(10) ** offerTokenInterface.decimals()), "amount too low");
        ...
        require(oldSellerBalance > offerTokenInterface.balanceOf(seller), "seller error");

        emit OfferAccepted(_offerId, seller, msg.sender, offerToken, buyerToken, _price, _amount);
    }
     ...
     *
     */
    function _isTransferValid(address _token, address _from, address _to, uint256 _amount)
        private
        view
        returns (bool)
    {
    ...
    function createOfferBatch(
        ...
        require(
            _buyerTokens.length == length && _buyers.length == length && _prices.length == length
                && _amounts.length == length,
            "length mismatch"
        );
        for (uint256 i = 0; i < length;) {
            createOffer(_offerTokens[i], _buyerTokens[i], _buyers[i], _prices[i], _amounts[i]);
            ++i;
        }
    }
        ...
        whenNotPaused
    {
        ...
        require(_prices.length == length && _amounts.length == length, "length mismatch");
        for (uint256 i = 0; i < length;) {
